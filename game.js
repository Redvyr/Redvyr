...
function gatheringDamage(resourceType = "") {
  const axeBonus =
    state.axeEquipped && (resourceType === "wood" || resourceType === "bush")
      ? 0.35
      : 0;
...
function updateBuyPanel() {
  ...
  const items = [
    ...
    },
    {
      ...
      desc: state.axeOwned ? "Owned. Double-click it in Inventory to equip." : "Boosts wood gathering when equipped.",
      cost: 250,
      img: images.axe.src,
      buy: () => {
        state.gold -= 250;
        ...
        toast("Basic Axe bought! Add it from Inventory.");
      },
...
function cleanHotbar() {
  ...
  state.hotbar = state.hotbar.map((item) => {
    if (item === "axe" && state.axeOwned) return item;
    if (item === "potion" && state.potions > 0) return item;
    return null;
  });

  if (!state.axeOwned) state.axeEquipped = false;
  if (state.axeEquipped && !state.hotbar.includes("axe")) state.axeEquipped = false;
}

function addItemToHotbar(itemType) {
  cleanHotbar();

  if (itemType === "axe" && !state.axeOwned) return;
  if (itemType === "potion" && state.potions <= 0) return;

  const alreadyThere = state.hotbar.indexOf(itemType);

  if (alreadyThere >= 0) {
    if (itemType === "axe") {
      state.axeEquipped = !state.axeEquipped;
      toast(state.axeEquipped ? "Axe equipped." : "Axe unequipped.");
    } else {
...
function updateHotbar() {
  ...
  state.hotbar.forEach((item, index) => {
    ...
    slot.type = "button";
    slot.title = item ? "Use/equip " + item : "Empty hotbar slot";

    if (item === "axe" && state.axeEquipped) {
    ...
    if (item) {
      ...
      count.textContent =
        item === "axe" ? (state.axeEquipped ? "EQ" : "") : "x" + state.potions;
...
function loadSave() {
  try {
    ...
    state.hotbar = Array.isArray(data.hotbar)
      ? data.hotbar.slice(0, 5)
      : [null, null, null, null, null];

    // Keep axes that were equipped in the previous version usable.
    if (state.axeOwned && state.axeEquipped && !state.hotbar.includes("axe")) {
      state.hotbar[0] = "axe";
    }
...
function updateInventoryPanel() {
  ...
  for (let i = 0; i < totalSlots; i++) {
    ...
    if (stacks[i]) {
      ...
      count.textContent =
        item.type === "axe"
          ? (state.axeEquipped ? "EQ" : "x1")
          : "x" + item.count;

      slot.appendChild(img);
      slot.appendChild(count);

      if (item.type === "axe" || item.type === "potion") {
        slot.classList.add("assignable");
        slot.title = "Double-click to add/use in hotbar.";

        if (state.hotbar.includes(item.type)) {
          slot.classList.add("hotbar-linked");
        }

        slot.addEventListener("dblclick", () => addItemToHotbar(item.type));
      }
    } else {
...
function hitResource(obj) {
  ...
  if (state.axeEquipped && (obj.action === "wood" || obj.action === "bush")) {
    player.axeSwing = 11;
  }
...
function drawEquippedTool(bob) {
  if (!state.axeEquipped || !state.axeOwned) return;
